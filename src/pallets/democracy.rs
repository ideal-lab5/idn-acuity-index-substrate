...
pub fn democracy_index_event(trees: Trees, block_number: u32, event_index: u32, event: subxt::events::EventDetails) -> Result<(), subxt::Error> {
    match event.variant_name() {
        ...
        "Started" => {
            ...
            index_event_ref_index(trees.clone(), event.ref_index, block_number, event_index);
            Ok(())
        },
        "Passed" => {
        ...
        "Seconded" => {
            ...
            index_event_proposal_index(trees.clone(), event.prop_index, block_number, event_index);
            Ok(())
        },
        "ProposalCanceled" => {

...
pub fn identity_index_event(trees: Trees, block_number: u32, event_index: u32, event: subxt::events::EventDetails) -> Result<(), subxt::Error> {
    match event.variant_name() {
        "IdentitySet" => {
            ...
            index_event_account_id(trees.clone(), event.who.clone(), block_number, event_index);
            Ok(())
        },
        "IdentityCleared" => {
        ...
        "JudgementRequested" => {
            ...
            index_event_account_id(trees.clone(), event.who, block_number, event_index);
            index_event_registrar_index(trees.clone(), event.registrar_index, block_number, event_index);
            Ok(())
        },
        ...
        "SubIdentityAdded" => {
            ...
            index_event_account_id(trees.clone(), event.main, block_number, event_index);
            Ok(())
        },
        "SubIdentityRemoved" => {

...
pub fn tips_index_event(
    ...
    match event.variant_name() {
        ...
        "TipClosing" => {
            let event = event
                .as_event::<polkadot::tips::events::TipClosing>()?
                .unwrap();
            indexer.index_event_tip_hash(event.tip_hash.into(), block_number, event_index);
            Ok(())
        }
        "TipClosed" => {
            let event = event
                .as_event::<polkadot::tips::events::TipClosed>()?
                .unwrap();
            indexer.index_event_tip_hash(event.tip_hash.into(), block_number, event_index);
        ...
        "TipRetracted" => {
            let event = event
                .as_event::<polkadot::tips::events::TipRetracted>()?
                .unwrap();
            indexer.index_event_tip_hash(event.tip_hash.into(), block_number, event_index);
            Ok(())
        }
        "TipSlashed" => {
            let event = event
                .as_event::<polkadot::tips::events::TipSlashed>()?
                .unwrap();
            indexer.index_event_tip_hash(event.tip_hash.into(), block_number, event_index);

...
pub fn proxy_index_event(
    ...
    match event.variant_name() {
        "PureCreated" => {
            let event = event
                .as_event::<polkadot::proxy::events::PureCreated>()?
                .unwrap();
        ...
        "Announced" => {
            let event = event
                .as_event::<polkadot::proxy::events::Announced>()?
                .unwrap();
        ...
        "ProxyAdded" => {
            let event = event
                .as_event::<polkadot::proxy::events::ProxyAdded>()?
                .unwrap();
        ...
        "ProxyRemoved" => {
            let event = event
                .as_event::<polkadot::proxy::events::ProxyRemoved>()?
                .unwrap();
            indexer.index_event_account_id(event.delegator, block_number, event_index);
            indexer.index_event_account_id(event.delegatee, block_number, event_index);
            Ok(())

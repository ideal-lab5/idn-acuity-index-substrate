...
pub fn elections_phragmen_index_event(
    indexer: &Indexer,
    block_number: u32,
    event_index: u32,
    ...
    match event.variant_name() {
        "NewTerm" => {
            let event = event
                .as_event::<polkadot::phragmen_election::events::NewTerm>()?
                .unwrap();
        ...
        "MemberKicked" => {
            let event = event
                .as_event::<polkadot::phragmen_election::events::MemberKicked>()?
                .unwrap();
        ...
        "Renounced" => {
            let event = event
                .as_event::<polkadot::phragmen_election::events::Renounced>()?
                .unwrap();
        ...
        "CandidateSlashed" => {
            let event = event
                .as_event::<polkadot::phragmen_election::events::CandidateSlashed>()?
                .unwrap();
            indexer.index_event_account_id(event.candidate, block_number, event_index);
            Ok(())
        }
        "SeatHolderSlashed" => {
            let event = event
                .as_event::<polkadot::phragmen_election::events::SeatHolderSlashed>()?
                .unwrap();
